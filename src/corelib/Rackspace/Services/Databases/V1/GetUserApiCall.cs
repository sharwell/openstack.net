namespace Rackspace.Services.Databases.V1
{
    using System;
    using OpenStack.Net;

    /// <summary>
    /// This class represents a prepared API call to get a user resource within a database instance.
    /// </summary>
    /// <seealso cref="RackspaceDatabaseServiceExtensions.PrepareGetUserAsync"/>
    /// <seealso cref="RackspaceDatabaseServiceExtensions.GetUserAsync"/>
    /// <threadsafety static="true" instance="false"/>
    /// <preliminary/>
    public class GetUserApiCall : DelegatingHttpApiCall<UserResponse>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="GetUserApiCall"/> class
        /// with the behavior provided by another <see cref="IHttpApiCall{T}"/> instance.
        /// </summary>
        /// <param name="httpApiCall">The <see cref="IHttpApiCall{T}"/> providing the behavior for the API call.</param>
        /// <exception cref="ArgumentNullException">If <paramref name="httpApiCall"/> is <see langword="null"/>.</exception>
        public GetUserApiCall(IHttpApiCall<UserResponse> httpApiCall)
            : base(httpApiCall)
        {
        }
    }
}