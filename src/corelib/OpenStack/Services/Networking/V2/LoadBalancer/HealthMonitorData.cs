namespace OpenStack.Services.Networking.V2.LoadBalancer
{
    ...
    public class HealthMonitorData : ExtensibleJsonObject
    {
        ...
        {
            Initialize(type, delay, timeout, maxRetries, httpMethod, null, null, null, null);
        }

        public HealthMonitorData(ProbeType type, TimeSpan? delay, TimeSpan? timeout, int? maxRetries, HttpMethod httpMethod, Uri urlPath, string expectedCodes, bool? adminStateUp, ProjectId projectId, params JProperty[] extensionData)
            : base(extensionData)
        {
            Initialize(type, delay, timeout, maxRetries, httpMethod, urlPath, expectedCodes, adminStateUp, projectId);
        }

        public HealthMonitorData(ProbeType type, TimeSpan? delay, TimeSpan? timeout, int? maxRetries, HttpMethod httpMethod, Uri urlPath, string expectedCodes, bool? adminStateUp, ProjectId projectId, IDictionary<string, JToken> extensionData)
            : base(extensionData)
        {
            Initialize(type, delay, timeout, maxRetries, httpMethod, urlPath, expectedCodes, adminStateUp, projectId);
        }

        private void Initialize(ProbeType type, TimeSpan? delay, TimeSpan? timeout, int? maxRetries, HttpMethod httpMethod, Uri urlPath, string expectedCodes, bool? adminStateUp, ProjectId projectId)
        {
            _type = type;
