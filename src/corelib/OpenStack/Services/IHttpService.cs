namespace OpenStack.Services
{
    ...
    /// <summary>
    /// This is the base interface for a service API exposed over HTTP. It defines the
    /// essential methods required for applications to write extension methods to support
    /// arbitrary custom features of the service, as opposed to requiring applications
    ...
    public interface IHttpService
    {
        ...
        /// <param name="parameters">A collection of parameters for binding the URI template in a call to <see cref="UriTemplate.BindByName{T}(Uri, IDictionary{string, T})"/>.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> that the task will observe.</param>
        /// <returns>A task continuation delegate which can be used to create an <see cref="HttpRequestMessage"/> following the completion of a task that obtains the base URI for a service.</returns>
        ...
        /// <param name="template">The <see cref="UriTemplate"/> for the target URI.</param>
        /// <param name="parameters">A collection of parameters for binding the URI template in a call to <see cref="UriTemplate.BindByName{T}(Uri, IDictionary{string, T})"/>.</param>
        /// <param name="body">A object modeling the body of the request. The service client implementation serializes this object, often using JSON.NET, to the form used in the HTTP request body.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> that the task will observe.</param>
        /// <returns>A task continuation delegate which can be used to create an <see cref="HttpRequestMessage"/> following the completion of a task that obtains the base URI for a service.</returns>
        /// <exception cref="ArgumentNullException">
        /// If <paramref name="method"/> is <see langword="null"/>.
