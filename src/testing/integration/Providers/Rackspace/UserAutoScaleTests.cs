    ...
    public class UserAutoScaleTests
    {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                foreach (ScalingGroup group in groups)
                {
                    ...
                    if (group.State.ActiveCapacity > 0)
                        prepTask = provider.SetGroupConfigurationAsync(group.Id, new GroupConfiguration(group.State.Name, TimeSpan.FromSeconds(60), 0, 0, new JObject()), AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                    else
                        prepTask = InternalTaskExtensions.CompletedTask();

                    cleanupTasks.Add(prepTask.ContinueWith(task => provider.DeleteGroupAsync(group.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null)).Unwrap());
                }
        ...
        public async Task TestCreateGroup()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                ScalingGroup scalingGroup = await provider.CreateGroupAsync(configuration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                Assert.IsNotNull(scalingGroup);
                Assert.IsNotNull(scalingGroup.Id);

                await provider.DeleteGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
            }
        }
        ...
        public async Task TestCreateGroupWithEntities()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                await provider.SetGroupConfigurationAsync(scalingGroup.Id, updatedConfiguration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        public async Task TestPauseGroup()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                await provider.PauseGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                await provider.PauseGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                await provider.ResumeGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                await provider.ResumeGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        public async Task TestSetLaunchConfiguration()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                await provider.SetLaunchConfigurationAsync(scalingGroup.Id, updatedConfiguration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        public async Task TestCreatePolicy()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                policy = await provider.CreatePolicyAsync(scalingGroup.Id, policyConfiguration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                ...
                await provider.DeletePolicyAsync(scalingGroup.Id, policy.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        private async Task TestCreatePolicyWithTimeFormat(string timeFormat)
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                /* Percentage Change At Time
                 */
                policyConfiguration = PolicyConfiguration.PercentageChangeAtTime("Percentage Change -50% At Time Policy", -50.0, TimeSpan.FromSeconds(60), DateTimeOffset.UtcNow.AddMinutes(30).ToString(timeFormat, CultureInfo.InvariantCulture));
                Assert.AreEqual(PolicyType.Schedule, policyConfiguration.PolicyType);
                policy = await provider.CreatePolicyAsync(scalingGroup.Id, policyConfiguration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                Assert.IsNotNull(policy);
                Assert.AreEqual(policyConfiguration.Change, policy.Change);
        ...
        public async Task TestSetPolicy()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                await provider.SetPolicyAsync(scalingGroup.Id, policy.Id, policyConfiguration, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        public async Task TestExecutePolicy()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                await provider.ExecutePolicyAsync(scalingGroup.Id, policy.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
        ...
        public async Task TestExecuteAnonymousWebhook()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                /* Cleanup
                 */
                await provider.SetGroupConfigurationAsync(scalingGroup.Id, new GroupConfiguration(name: groupName, cooldown: TimeSpan.Zero, minEntities: 0, maxEntities: 0, metadata: new JObject()), AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
                await provider.DeleteGroupAsync(scalingGroup.Id, AsyncCompletionOption.RequestCompleted, cancellationTokenSource.Token, null);
            }
        }
        ...
        public async Task TestCreateWebhookRange()
        {
            ...
            using (CancellationTokenSource cancellationTokenSource = new CancellationTokenSource(TestTimeout(TimeSpan.FromSeconds(300))))
            {
                ...
                Webhook[] webhooks = await provider.CreateWebhookRangeAsync(scalingGroup.Id, scalingGroup.ScalingPolicies[0].Id, webhookConfigurations, cancellationTokenSource.Token);
                Assert.IsNotNull(webhooks);
                Assert.AreEqual(webhookConfigurations.Length, webhooks.Length);
