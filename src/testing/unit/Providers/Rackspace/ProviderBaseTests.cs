...
namespace OpenStackNet.Testing.Unit.Providers.Rackspace
{
    ...
    public class ProviderBaseTests
    {
        ...
        {
            ServiceCatalog serviceCatalog = JsonConvert.DeserializeObject<ServiceCatalog>(@"{ name : """ + _testService + @""", endpoints : [{region:""DFW""}, {region:""ORD""}]}");
        ...
        {
            ServiceCatalog serviceCatalog = JsonConvert.DeserializeObject<ServiceCatalog>(@"{ name : """ + _testService + @""", endpoints : [{region:""LON""}, {region:""LON2""}]}");
